// src/components/DS_ITAM.js

import React from 'react'

function DS_ITAM() {
  return (
    <div>
      <h2>Early Warning System of Dropouts at Undergraduate Levels</h2>
        <p>
            Along with Adolfo de Unánue, Liliana Millán, Paola Mejía, and Elizabeth Rodriguez
        </p>
      <p align="justify">
      Early Warning System to identify undergraduate ITAM students in scholar vulnerability in the next
      two years. In particular, we used metrics based on assistive or punitive actions to optimize the 
      models, performed temporal cross-validation, and used existing methodologies for analyzing algorithmic 
      bias and fairness among student groups. My responsibilities included structuring databases, exploring 
      data patterns, and implementing training and validation of machine learning models to generate risk 
      score predictions for each student.
      </p>
    </div>
  )
}

export default DS_ITAM;
